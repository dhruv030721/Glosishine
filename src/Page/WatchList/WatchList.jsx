...
export const WatchList = () => {
  ...
  useEffect(() => {
    const fetchWatchlist = async () => {
      try {
        const response = await getFavProduct(); // Call the API
        console.log("API response:", response); // Log the full response for debugging

        if (response && response.success) {
          dispatch(setWatchlist(response.data)); // Set data to Redux state
        } else {
          // Log more details if response.success is false or undefined
          console.error(
  ...
  return (
    <div className="flex flex-wrap justify-around p-5 my-10 gap-6">
        ...
        >
          ...
          >
            <img
              ...
              src={product.product_img || product.images[0]}
