...
export async function setContentItem(array, type, section_id) {
    ...
    const response = await apiConnector("POST", adminendpoints.ADD_CONTENT_ITEM, formData);
    return response;
}
...
export async function deleteContentItem(id) {
    ...
    const response = await apiConnector("GET", adminendpoints.DELETE_CONTENT_ITEM + id);
    return response;
}
